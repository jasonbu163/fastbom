...
class DXFProcessor:
    ...
    def process_dxf_file(file_path: Path, num: int, output_dir: Path) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                msp.add_text(
                    file_name_to_insert,
                    dxfattribs={'layer': '0', 'height': text_height, 'color': 2}
    ...
    def merge_directory_to_dxf(input_dir: Path, output_file: Path) -> Tuple[bool, str]:
        ...
        for layer in merged_doc.layers:
            if layer.dxf.name not in layers_buff:
                layer.off()
    ...
    def merge_by_thickness(source_dir: Path, output_dir: Path) -> Tuple[int, int, List[str]]:
        ...
        # 遍历材料层 (例如: 铝板, 不锈钢板)
        for material_path in source_dir.iterdir():
            if not material_path.is_dir():
                continue
            
            # 遍历厚度层 (例如: 2.0, 3.0)
            for thickness_path in material_path.iterdir():
                if not thickness_path.is_dir():
                    continue
                
                logs.append(f"📦 正在合并组: {material_path.name} - {thickness_path.name}")
                
                # 生成输出文件名: 材料_厚度_合并.dxf
                output_filename = f"{material_path.name}_{thickness_path.name}_merged.dxf"
                target_file = output_dir / output_filename
                
                # 调用现有的合并单个目录的方法
                success, msg = DXFProcessor.merge_directory_to_dxf(thickness_path, target_file)
...
class WorkerThread(QThread):
    ...
    def _run_dxf_merge(self):
        ...
        # 从分类结果目录读取，输出到合并目录
        source_dir = self.classifier.out_dir  # 1_分类结果
        output_dir = self.classifier.merge_dir  # 3_合并文件
...
class MainWindow(QMainWindow):
    ...
    def _create_step4(self, layout):
        ...
        info = QLabel("功能：在图纸中添加标注（bom表标注数量和图号）")
    ...
    def _create_step5(self, layout):
        ...
        info = QLabel("功能：将同一材料和厚度的DXF文件合并到一个文件，添加标注，隐藏非0图层")
        info.setStyleSheet("color: #666;")
    ...
    def _apply_styles(self):
        """应用全局样式 - 使用qt_material"""
        # 移除所有自定义样式，使用qt_material主题
        pass
    ...
    def _on_merge_dxf_finished(self, success, msg):
        ...
        if success:
            QMessageBox.information(self, "完成", msg)
            self.classifier._open_folder(self.classifier.merge_dir)
        else:
            QMessageBox.warning(self, "失败", msg)
...
def main():
    ...
    # 设置应用图标（可选）
    app.setWindowIcon(QIcon('../static/efficacy_researching_settings_icon_152066.ico'))
