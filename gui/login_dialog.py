...
class LoginDialog(QDialog):
    def __init__(self, auth_service: AuthService, settings: AppSettings, settings_store):
        ...
        self.setWindowTitle("登录 FastBOM")
        ...
        title = QLabel("登录 FastBOM")
        subtitle = QLabel("后端账号可解锁远程物料库；离线 admin 仅用于后端不可用时进入软件。")
        ...
        api_settings_btn = QPushButton("后端设置")
        ...
        self.username_edit.setPlaceholderText("后端账号或 admin")
        ...
        self.backend_login_btn = QPushButton("后端账号登录")
    ...
    def _show_settings_dialog(self) -> None:
        ...
        form.addRow("后端 API 地址", api_url_edit)
        ...
        self._refresh_api_state_label()
        QMessageBox.information(self, "后端设置", "后端设置已保存。")
    ...
    def _refresh_api_state_label(self) -> None:
        ...
        if api_url:
            self.api_state_label.setText(f"后端 API：{api_url}")
        else:
            self.api_state_label.setText("后端 API：未配置，可先使用离线登录")
