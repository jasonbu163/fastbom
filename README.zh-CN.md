...
- 设置页用于配置本地工作流默认值，例如默认 BOM 列、模板目录、输出目录、
  DXF 参数、SolidWorks 可见性、库存导出文件名前缀，以及离线 `admin` 密码。
...
当前桌面端已经接入 PMMS 后端认证，并提供板材物料库存管理和用户管理页面。
Qt 客户端应遵循 `pmms-integration-materials/` 中的后端契约，不要在本地自行
发明请求或响应结构。
...
- 如果 Qt 客户端使用离线 `admin` 身份登录，本次会话必须禁用远程库存和用户管理功能。
- 只有正常后端认证通过的用户会话，才允许使用远程库存、物料规格和用户管理工作流。
...
- 使用离线 `admin` 登录可以进入桌面软件，但会禁用远程板材物料库存和用户管理功能。
...
- 板材物料库存管理页，包含库存分页、筛选、物料规格、XLSX 导入导出、入库和扣减 / 领用。
- 本地处理页，内部包含准备与识别、分类转换、DXF 标注、DXF 合并四个二级页面。
- 设置页。
- 用户管理页，供具备后端管理权限的账号维护普通用户。
